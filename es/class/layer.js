  ...
  _createClass(Layer, null, [
    ...
    },
    {
      ...
      value: function detectAndUpdateAreas(state, layerID) {
        ...
        var innerCyclesByVerticesArrayIndex = GraphInnerCycles.calculateInnerCycles(
          verticesArray,
          linesArray
        )

        var innerCyclesByVerticesID = new List(
        ...
        innerCyclesByVerticesID = innerCyclesByVerticesID.map(function (area) {
          return GraphInnerCycles.isClockWiseOrder(
        ...
        for (i = 0; i < verticesCoordsForArea.length; i++) {
          ...
          for (j = 0; j < verticesCoordsForArea.length; j++) {
            if (i !== j) {
              var isHole = GeometryUtils.ContainsPoint(
                areaVerticesList,
    ...
    },
    {
      ...
      value: function setPropertiesOnSelected(state, layerID, properties) {
        ...
        selected.lines.forEach(function (lineID) {
          return (state = Line.setProperties(state, layerID, lineID, properties)
            .updatedState)
        })
        selected.holes.forEach(function (holeID) {
          return (state = Hole.setProperties(state, layerID, holeID, properties)
            .updatedState)
        })
        selected.areas.forEach(function (areaID) {
          return (state = Area.setProperties(state, layerID, areaID, properties)
            .updatedState)
        })
        selected.items.forEach(function (itemID) {
          return (state = Item.setProperties(state, layerID, itemID, properties)
            .updatedState)
        })
    ...
    },
    {
      ...
      value: function updatePropertiesOnSelected(state, layerID, properties) {
        ...
        selected.lines.forEach(function (lineID) {
          return (state = Line.updateProperties(
            state,
            layerID,
            lineID,
            properties
        ...
        selected.holes.forEach(function (holeID) {
          return (state = Hole.updateProperties(
            state,
            layerID,
            holeID,
            properties
        ...
        selected.areas.forEach(function (areaID) {
          return (state = Area.updateProperties(
            state,
            layerID,
            areaID,
            properties
        ...
        selected.items.forEach(function (itemID) {
          return (state = Item.updateProperties(
            state,
            layerID,
            itemID,
            properties
    ...
    },
    {
      ...
      value: function setAttributesOnSelected(state, layerID, attributes) {
        ...
        selected.lines.forEach(function (lineID) {
          return (state = Line.setAttributes(state, layerID, lineID, attributes)
            .updatedState)
        })
        selected.holes.forEach(function (holeID) {
          return (state = Hole.setAttributes(state, layerID, holeID, attributes)
            .updatedState)
        })
        selected.items.forEach(function (itemID) {
          return (state = Item.setAttributes(state, layerID, itemID, attributes)
            .updatedState)
        })
