...
class Layer {
  ...
  static setPropertiesOnSelected(state, layerID, properties) {
    ...
    selected.lines.forEach(
      (lineID) =>
        (state = Line.setProperties(state, layerID, lineID, properties)
          .updatedState)
    )
    selected.holes.forEach(
      (holeID) =>
        (state = Hole.setProperties(state, layerID, holeID, properties)
          .updatedState)
    )
    selected.areas.forEach(
      (areaID) =>
        (state = Area.setProperties(state, layerID, areaID, properties)
          .updatedState)
    )
    selected.items.forEach(
      (itemID) =>
        (state = Item.setProperties(state, layerID, itemID, properties)
          .updatedState)
    )
  ...
  static updatePropertiesOnSelected(state, layerID, properties) {
    ...
    selected.lines.forEach(
      (lineID) =>
        (state = Line.updateProperties(state, layerID, lineID, properties)
          .updatedState)
    )
    selected.holes.forEach(
      (holeID) =>
        (state = Hole.updateProperties(state, layerID, holeID, properties)
          .updatedState)
    )
    selected.areas.forEach(
      (areaID) =>
        (state = Area.updateProperties(state, layerID, areaID, properties)
          .updatedState)
    )
    selected.items.forEach(
      (itemID) =>
        (state = Item.updateProperties(state, layerID, itemID, properties)
          .updatedState)
    )
