...
var ElementEditor = (function (_Component) {
  ...
  function ElementEditor(props, context) {
    ...
    var _this = _possibleConstructorReturn(
      this,
      (ElementEditor.__proto__ || Object.getPrototypeOf(ElementEditor)).call(
        this,
        props,
        context
      )
    ...
    _this.state = {
      attributesFormData: _this.initAttrData(
        _this.props.element,
        _this.props.layer,
        _this.props.state
      ),
      propertiesFormData: _this.initPropData(
        _this.props.element,
        _this.props.layer,
        _this.props.state
      ),
  ...
  _createClass(ElementEditor, [
    ...
    },
    {
      ...
      value: function initAttrData(element, layer, state) {
        element =
          _typeof(element.misc) === 'object'
            ? element.set('misc', new _immutable.Map(element.misc))
            : element
        ...
        switch (element.prototype) {
          ...
          case 'lines': {
            ...
            var distance = _export.GeometryUtils.pointsDistance(
              v_a.x,
              v_a.y,
              v_b.x,
              v_b.y
            ...
            var _length = (0, _convertUnits2.default)(distance)
          ...
          case 'holes': {
            ...
            var lineLength = _export.GeometryUtils.pointsDistance(
              x0,
              y0,
              x1,
              y1
            ...
            var _lengthA = (0, _convertUnits2.default)(startAt)
              .from(this.context.catalog.unit)
              .to(_unitA)
            ...
            var _lengthB = (0, _convertUnits2.default)(endAt)
              .from(this.context.catalog.unit)
              .to(_unitB)
          ...
          case 'lines': {
            switch (attributeName) {
              case 'lineLength': {
                ...
                var _GeometryUtils$orderV = _export.GeometryUtils.orderVertices(
                    [v_0, v_1]
                  ),
                  _GeometryUtils$orderV2 = _slicedToArray(
                    _GeometryUtils$orderV,
                    2
                  ),
                  v_a = _GeometryUtils$orderV2[0],
                  v_b = _GeometryUtils$orderV2[1]

                var v_b_new = _export.GeometryUtils.extendLine(
                  v_a.x,
                  v_a.y,
                  v_b.x,
                  v_b.y,
                ...
                attributesFormData = attributesFormData.withMutations(function (
                  attr
                ) {
                  attr.set(
              ...
              case 'vertexTwo': {
                attributesFormData = attributesFormData.withMutations(function (
                  ...
                  var newDistance = _export.GeometryUtils.verticesDistance(
                    attr.get('vertexOne'),
                    attr.get('vertexTwo')
                  )
                  ...
                  attr.mergeIn(
                    ...
                    attr.get('lineLength').merge({
                      length: newDistance,
                      _length: (0, _convertUnits2.default)(newDistance)
                        .from(_this2.context.catalog.unit)
          ...
          case 'holes': {
            switch (attributeName) {
              case 'offsetA': {
                ...
                var orderedVertices = _export.GeometryUtils.orderVertices([
                ...
                var alpha = _export.GeometryUtils.angleBetweenTwoPoints(
                  x0,
                  y0,
                  x1,
                  y1
                ...
                var widthLength = this.props.element.properties
                  .get('width')
                  .get('length')
                ...
                var offset = _export.GeometryUtils.pointPositionOnLineSegment(
                  x0,
                  y0,
                  x1,
                  y1,
                ...
                var endAt = _export.MathUtils.toFixedFloat(
                ...
                var offsetB = new _immutable.Map({
                  length: endAt,
                  _length: (0, _convertUnits2.default)(endAt)
                    .from(this.context.catalog.unit)
                    .to(offsetUnit),
                  _unit: offsetUnit,
                })
                ...
                var offsetAttribute = new _immutable.Map({
                  length: _export.MathUtils.toFixedFloat(
                    lengthValue,
                    PRECISION
                  ),
                  _unit: value.get('_unit'),
                  _length: _export.MathUtils.toFixedFloat(
                    (0, _convertUnits2.default)(lengthValue)
                      .from(this.context.catalog.unit)
                      .to(value.get('_unit')),
                    PRECISION
                  ),
              ...
              case 'offsetB': {
                ...
                var _orderedVertices2 = _export.GeometryUtils.orderVertices([
                  this.props.layer.vertices.get(_line.vertices.get(0)),
                  this.props.layer.vertices.get(_line.vertices.get(1)),
                ])
                ...
                var _widthLength = this.props.element.properties
                  .get('width')
                  .get('length')
                ...
                _lengthValue = Math.min(
                  _lengthValue,
                  _lineLength - _widthLength
                )

                var _xp =
                  _x2 - (_lengthValue + _halfWidthLength) * Math.cos(_alpha)
                var _yp =
                  _y2 - (_lengthValue + _halfWidthLength) * Math.sin(_alpha)
                ...
                var startAt = _export.MathUtils.toFixedFloat(
                  _lineLength * _offset - _halfWidthLength,
                  PRECISION
                )
                ...
                var offsetA = new _immutable.Map({
                  length: startAt,
                  _length: (0, _convertUnits2.default)(startAt)
                    .from(this.context.catalog.unit)
                    .to(_offsetUnit),
                  _unit: _offsetUnit,
                })
                ...
                var _offsetAttribute = new _immutable.Map({
                  ...
                  _unit: value.get('_unit'),
                  _length: _export.MathUtils.toFixedFloat(
                    (0, _convertUnits2.default)(_lengthValue)
                      .from(this.context.catalog.unit)
                      .to(value.get('_unit')),
                    PRECISION
                  ),
