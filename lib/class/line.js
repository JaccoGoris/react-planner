  ...
  _createClass(Line, null, [
    {
      ...
      value: function create(state, layerID, type, x0, y0, x1, y1, properties) {
        ...
        var _Vertex$add = _export.Vertex.add(
            state,
            layerID,
            x0,
            y0,
        ...
        var _Vertex$add2 = _export.Vertex.add(
            stateV0,
            layerID,
            x1,
            y1,
        ...
        var line = state.catalog.factoryElement(
          ...
          {
            id: lineID,
            name: _export2.NameGenerator.generateName(
              'lines',
    ...
    },
    {
      ...
      value: function select(state, layerID, lineID) {
        ...
        state = _export.Layer.selectElement(state, layerID, 'lines', lineID)
          .updatedState
        state = _export.Layer.selectElement(
          state,
          layerID,
          'vertices',
          line.vertices.get(0)
      ...
      value: function remove(state, layerID, lineID) {
        ...
        if (line) {
          ...
          line.holes.forEach(function (holeID) {
            return (state = _export.Hole.remove(state, layerID, holeID)
              .updatedState)
          })
          state = _export.Layer.removeElement(state, layerID, 'lines', lineID)
            .updatedState

          line.vertices.forEach(function (vertexID) {
            return (state = _export.Vertex.remove(
              state,
              layerID,
              vertexID,
              'lines',
          ...
          state.getIn(['scene', 'groups']).forEach(function (group) {
            return (state = _export.Group.removeElement(
              state,
              group.id,
              layerID,
              'lines',
    ...
    },
    {
      ...
      value: function unselect(state, layerID, lineID) {
        ...
        if (line) {
          ...
          state = _export.Layer.unselect(state, layerID, 'lines', lineID)
            .updatedState
        }
    ...
    },
    {
      ...
      value: function split(state, layerID, lineID, x, y) {
        ...
        var splitPointOffset = _export2.GeometryUtils.pointPositionOnLineSegment(
          x0,
          y0,
          x1,
          y1,
        ...
        lineGroups.forEach(function (group) {
          state = _export.Group.addElement(
            state,
            group.id,
            layerID,
            'lines',
        ...
        pointsPair.forEach(function (_ref5) {
          ...
          if (holes) {
            holes.forEach(function (holeWithOffsetPoint) {
              ...
              if (
                _export2.GeometryUtils.isPointOnLineSegment(
                  x1,
                  y1,
                  x2,
                  y2,
                ...
                var newOffset = _export2.GeometryUtils.pointPositionOnLineSegment(
                  x1,
                  y1,
                  x2,
                  y2,
        ...
        state = state
          ...
          .reduce(function (reducedState, line) {
            ...
            var hasCommonEndpoint =
              _export2.GeometryUtils.samePoints(v0, points[0]) ||
              _export2.GeometryUtils.samePoints(v0, points[1]) ||
              _export2.GeometryUtils.samePoints(v1, points[0]) ||
              _export2.GeometryUtils.samePoints(v1, points[1])

            var intersection = _export2.GeometryUtils.twoLineSegmentsIntersection(
              points[0],
              points[1],
              v0,
              v1
            ...
            if (intersection.type === 'colinear') {
              ...
              reducedState
                ...
                .forEach(function (holeID) {
                  ...
                  var oldLineLength = _export2.GeometryUtils.pointsDistance(
                    v0.x,
                    v0.y,
                    v1.x,
                    v1.y
                  )
                  var offset = _export2.GeometryUtils.samePoints(
                    orderedVertices[1],
                    line.vertices.get(1)
                  )
                    ? 1 - hole.offset
                    : hole.offset
                  var offsetPosition = _export2.GeometryUtils.extendLine(
                    v0.x,
                    v0.y,
                    v1.x,
                    v1.y,
              ...
              reducedState = _this2.remove(reducedState, layerID, line.id)
                .updatedState
    ...
    },
    {
      ...
      value: function replaceVertex(state, layerID, lineID, vertexIndex, x, y) {
        ...
        state = _export.Vertex.remove(state, layerID, vertexID, 'lines', lineID)
          .updatedState

        var _Vertex$add3 = _export.Vertex.add(
            state,
            layerID,
            x,
            y,
    ...
    },
    {
      ...
      value: function beginDrawingLine(state, layerID, x, y) {
        var snapElements = _export2.SnapSceneUtils.sceneSnapElements(
          state.scene,
          new _immutable.List(),
          state.snapMask
        )
        ...
        if (state.snapMask && !state.snapMask.isEmpty()) {
          snap = _export2.SnapUtils.nearestSnap(
            snapElements,
            x,
            y,
            state.snapMask
    ...
    },
    {
      ...
      value: function endDrawingLine(state, x, y) {
        if (state.snapMask && !state.snapMask.isEmpty()) {
          var snap = _export2.SnapUtils.nearestSnap(
            state.snapElements,
            x,
            y,
            state.snapMask
    ...
    },
    {
      ...
      value: function updateDraggingLine(state, x, y) {
        ...
        var curSnap0 = null,
          curSnap1 = null
        if (state.snapMask && !state.snapMask.isEmpty()) {
          curSnap0 = _export2.SnapUtils.nearestSnap(
            snapElements,
            newVertex0X,
            newVertex0Y,
            state.snapMask
          )
          curSnap1 = _export2.SnapUtils.nearestSnap(
            snapElements,
            newVertex1X,
            newVertex1Y,
            state.snapMask
        ...
        state = state.merge({
          ...
          scene: state.scene.updateIn(['layers', layerID], function (layer) {
            return layer.withMutations(function (layer) {
              ...
              layer.updateIn(['vertices', lineVertices.get(0)], function (
                vertex
              ) {
                return vertex.merge({ x: newVertex0X, y: newVertex0Y })
              })
              layer.updateIn(['vertices', lineVertices.get(1)], function (
                vertex
              ) {
    ...
    },
    {
      ...
      value: function endDraggingLine(state, x, y) {
        ...
        if (state.snapMask && !state.snapMask.isEmpty()) {
          var curSnap0 = _export2.SnapUtils.nearestSnap(
            state.snapElements,
            newVertex0X,
            newVertex0Y,
            state.snapMask
          )
          var curSnap1 = _export2.SnapUtils.nearestSnap(
            state.snapElements,
            newVertex1X,
            newVertex1Y,
            state.snapMask
        ...
        state = _export.Layer.mergeEqualsVertices(
          state,
          layerID,
          line.vertices.get(0)
        ).updatedState
        ...
        if (
          !_export2.GeometryUtils.samePoints(
    ...
    },
    {
      ...
      value: function setJsProperties(state, layerID, lineID, properties) {
        return this.setProperties(
          ...
          layerID,
          lineID,
          (0, _immutable.fromJS)(properties)
        )
      },
    },
    {
      ...
      value: function setAttributes(state, layerID, lineID, lineAttributes) {
        ...
        state = state
          ...
          .mergeIn(
            ...
            new _immutable.Map({ _unitLength: lineLength._unit })
          )

        state = _export.Layer.mergeEqualsVertices(state, layerID, vertexOne.id)
          .updatedState
    ...
    },
    {
      ...
      value: function setVerticesCoords(
        ...
        var line = state.getIn(['scene', 'layers', layerID, 'lines', lineID])
        state = _export.Vertex.setAttributes(
          state,
          layerID,
          line.vertices.get(0),
          new _immutable.Map({ x: x1, y: y1 })
        ).updatedState
        state = _export.Vertex.setAttributes(
          state,
          layerID,
          line.vertices.get(1),
          new _immutable.Map({ x: x2, y: y2 })
        ).updatedState
