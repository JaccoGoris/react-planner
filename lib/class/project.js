  ...
  _createClass(Project, null, [
    ...
    },
    {
      ...
      value: function openCatalog(state) {
        state = this.setMode(state, _constants.MODE_VIEWING_CATALOG)
          .updatedState
    ...
    },
    {
      ...
      value: function setProperties(state, layerID, properties) {
        state = _export2.Layer.setPropertiesOnSelected(
          state,
          layerID,
          properties
        ).updatedState
    ...
    },
    {
      ...
      value: function updateProperties(state, layerID, properties) {
        state = _export2.Layer.updatePropertiesOnSelected(
          state,
          layerID,
          properties
        ).updatedState
    ...
    },
    {
      ...
      value: function setItemsAttributes(state, attributes) {
        ...
        state.getIn(['scene', 'layers']).forEach(function (layer) {
          state = _export2.Layer.setAttributesOnSelected(
            state,
            layer.id,
            attributes
          ).updatedState
    ...
    },
    {
      ...
      value: function remove(state) {
        ...
        selectedLines.forEach(function (lineID) {
          state = _export2.Line.remove(state, selectedLayer, lineID)
            .updatedState
        })
        selectedHoles.forEach(function (holeID) {
          state = _export2.Hole.remove(state, selectedLayer, holeID)
            .updatedState
        })
        selectedItems.forEach(function (itemID) {
          state = _export2.Item.remove(state, selectedLayer, itemID)
            .updatedState
        })

        state = _export2.Layer.detectAndUpdateAreas(state, selectedLayer)
          .updatedState
    ...
    },
    {
      ...
      value: function changeCatalogPage(state, oldPage, newPage) {
        state = state
          ...
          .updateIn(['catalog', 'path'], function (path) {
