  ...
  _createClass(Group, null, [
        ...
        layerList.entrySeq().forEach(function (_ref) {
          ...
          if (lines)
            lines.forEach(function (lineID) {
              state = _export.Line.select(state, groupLayerID, lineID)
                .updatedState
            })
          if (holes)
            holes.forEach(function (holeID) {
              state = _export.Hole.select(state, groupLayerID, holeID)
                .updatedState
            })
          if (items)
            items.forEach(function (itemID) {
              state = _export.Item.select(state, groupLayerID, itemID)
                .updatedState
            })
          if (areas)
            areas.forEach(function (areaID) {
              state = _export.Area.select(state, groupLayerID, areaID)
                .updatedState
            })
        })
    ...
    },
    {
      ...
      value: function create(state) {
        ...
        state = state.setIn(
          ...
          new _models.Group({ id: groupID, name: groupID })
        )
        ...
        layerList.entrySeq().forEach(function (_ref3) {
          ...
          if (lines)
            lines.forEach(function (lineID) {
              ...
              var _GeometryUtils$midPoi = _export2.GeometryUtils.midPoint(
                  x1,
                  y1,
                  x2,
                  y2
          ...
          if (holes)
            holes.forEach(function (holeID) {
              ...
              var _GeometryUtils$extend = _export2.GeometryUtils.extendLine(
                  x1,
                  y1,
                  x2,
                  y2,
                  hole.offset *
                    _export2.GeometryUtils.pointsDistance(x1, y1, x2, y2)
                ),
          ...
          if (areas)
            areas.forEach(function (areaID) {
              ...
              var _GeometryUtils$vertic = _export2.GeometryUtils.verticesMidPoint(
                  areaVertices
                ),
    ...
    },
    {
      ...
      value: function removeElement(
        ...
        state = state.setIn(
          ['scene', 'groups', groupID, 'elements', layerID, elementPrototype],
          actualList.filterNot(function (el) {
            return el === elementID
          })
        )
    ...
    },
    {
      ...
      value: function removeAndDeleteElements(state, groupID) {
        ...
        layerList.entrySeq().forEach(function (_ref5) {
          ...
          if (lines) {
            lines.forEach(function (lineID) {
              state = _export.Line.remove(state, groupLayerID, lineID)
                .updatedState
              state = _export.Layer.detectAndUpdateAreas(state, groupLayerID)
                .updatedState
            })
          }
          ...
          if (holes)
            holes.forEach(function (holeID) {
              state = _export.Hole.remove(state, groupLayerID, holeID)
                .updatedState
            })
          if (items)
            items.forEach(function (itemID) {
              state = _export.Item.remove(state, groupLayerID, itemID)
                .updatedState
            })
          //( actually ) no effect by area's destruction
          if (false && areas)
            areas.forEach(function (areaID) {
              state = _export.Area.remove(state, groupLayerID, areaID)
                .updatedState
            })
        })
          ...
          if (lines) {
            ...
            for (var vertexID in vertices) {
              ...
              state = _export.Vertex.setAttributes(
                ...
                groupLayerID,
                vertexID,
                new _immutable.Map({ x: xV + deltaX, y: yV + deltaY })
              ).updatedState
            }

            //need to be separated from setAttributes cycle
            for (var _vertexID in vertices) {
              state = _export.Vertex.beginDraggingVertex(
                state,
                groupLayerID,
                _vertexID
              ).updatedState
          ...
          if (items)
            state = items
              ...
              .reduce(function (newState, item) {
                ...
                return _export.Item.setAttributes(
                  newState,
                  groupLayerID,
                  item.id,
                  new _immutable.Map({ x: xI + deltaX, y: yI + deltaY })
                ).updatedState
              }, state)
    ...
    },
    {
      ...
      value: function rotate(state, groupID, newAlpha) {
        ...
        state = Group.setAttributes(
          state,
          groupID,
          new _immutable.Map({ rotation: newAlpha })
        ).updatedState
        ...
        layerList.entrySeq().forEach(function (_ref9) {
          ...
          if (lines) {
            ...
            for (var vertexID in vertices) {
              ...
              var _GeometryUtils$rotate = _export2.GeometryUtils.rotatePointAroundPoint(
                  xV,
                  yV,
                  barX,
                  barY,
              ...
              state = _export.Vertex.setAttributes(
                ...
                groupLayerID,
                vertexID,
                new _immutable.Map({ x: newX, y: newY })
              ).updatedState
            }
            //need to be separated from setAttributes cycle
          ...
          if (items)
            state = items
              ...
              .reduce(function (newState, item) {
                ...
                var _GeometryUtils$rotate2 = _export2.GeometryUtils.rotatePointAroundPoint(
                    xI,
                    yI,
                    barX,
                    barY,
                ...
                return _export.Item.setAttributes(
                  ...
                  groupLayerID,
                  item.id,
                  new _immutable.Map({ x: newX, y: newY, rotation: rI + alpha })
                ).updatedState
              }, state)
