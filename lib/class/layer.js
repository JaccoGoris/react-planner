  ...
  _createClass(Layer, null, [
    ...
    },
    {
      ...
      value: function detectAndUpdateAreas(state, layerID) {
        ...
        var innerCyclesByVerticesArrayIndex = _export2.GraphInnerCycles.calculateInnerCycles(
          verticesArray,
          linesArray
        )

        var innerCyclesByVerticesID = new _immutable.List(
          innerCyclesByVerticesArrayIndex
        ).map(function (cycle) {
          return new _immutable.List(
            cycle.map(function (vertexIndex) {
        ...
        innerCyclesByVerticesID = innerCyclesByVerticesID.map(function (area) {
          return _export2.GraphInnerCycles.isClockWiseOrder(
            area.map(function (vertexID) {
              return state.getIn([
                'scene',
                'layers',
                layerID,
        ...
        innerCyclesByVerticesID.forEach(function (cycle, ind) {
          ...
          if (areaInUse) {
            ...
            var resultAdd = _export.Area.add(
              state,
              layerID,
              'area',
              areaVerticesCoords,
        ...
        var verticesCoordsForArea = areaIDs.map(function (id) {
          var vertices = state
            ...
            .vertices.map(function (vertexID) {
        ...
        for (i = 0; i < verticesCoordsForArea.length; i++) {
          ...
          var areaVerticesList = verticesCoordsForArea[i].vertices
            .flatten()
            .toArray()
          for (j = 0; j < verticesCoordsForArea.length; j++) {
            if (i !== j) {
              var isHole = _export2.GeometryUtils.ContainsPoint(
                areaVerticesList,
        ...
        var updatedState = state
          ...
          .reduce(function (newState, line) {
            ...
            if (_export2.GeometryUtils.verticesDistance(v0, v1) === 0) {
              newState = _export.Line.remove(newState, layerID, line.id)
                .updatedState
            }
    ...
    },
    {
      ...
      value: function mergeEqualsVertices(state, layerID, vertexID) {
        ...
        var doubleVertices = state
          .getIn(['scene', 'layers', layerID, 'vertices'])
          .filter(function (v) {
            return (
              v.id !== vertexID && _export2.GeometryUtils.samePoints(vertex, v) // &&
        ...
        doubleVertices.forEach(function (doubleVertex) {
          var reduced = doubleVertex.lines.reduce(function (
            reducedState,
            lineID
          ) {
            ...
            reducedState = _export.Vertex.addElement(
              reducedState,
              layerID,
              vertexID,
              'lines',
          ...
          var biReduced = doubleVertex.areas.reduce(function (
            reducedState,
            areaID
          ) {
            reducedState = reducedState.updateIn(
    ...
    },
    {
      ...
      value: function setPropertiesOnSelected(state, layerID, properties) {
        ...
        selected.lines.forEach(function (lineID) {
          return (state = _export.Line.setProperties(
            state,
            layerID,
            lineID,
            properties
        ...
        selected.holes.forEach(function (holeID) {
          return (state = _export.Hole.setProperties(
            state,
            layerID,
            holeID,
            properties
        ...
        selected.areas.forEach(function (areaID) {
          return (state = _export.Area.setProperties(
            state,
            layerID,
            areaID,
            properties
        ...
        selected.items.forEach(function (itemID) {
          return (state = _export.Item.setProperties(
            state,
            layerID,
            itemID,
            properties
    ...
    },
    {
      ...
      value: function updatePropertiesOnSelected(state, layerID, properties) {
        ...
        selected.lines.forEach(function (lineID) {
          return (state = _export.Line.updateProperties(
            state,
            layerID,
            lineID,
            properties
        ...
        selected.holes.forEach(function (holeID) {
          return (state = _export.Hole.updateProperties(
            state,
            layerID,
            holeID,
            properties
        ...
        selected.areas.forEach(function (areaID) {
          return (state = _export.Area.updateProperties(
            state,
            layerID,
            areaID,
            properties
        ...
        selected.items.forEach(function (itemID) {
          return (state = _export.Item.updateProperties(
            state,
            layerID,
            itemID,
            properties
    ...
    },
    {
      ...
      value: function setAttributesOnSelected(state, layerID, attributes) {
        ...
        selected.lines.forEach(function (lineID) {
          return (state = _export.Line.setAttributes(
            state,
            layerID,
            lineID,
            attributes
        ...
        selected.holes.forEach(function (holeID) {
          return (state = _export.Hole.setAttributes(
            state,
            layerID,
            holeID,
            attributes
        ...
        selected.items.forEach(function (itemID) {
          return (state = _export.Item.setAttributes(
            state,
            layerID,
            itemID,
            attributes
