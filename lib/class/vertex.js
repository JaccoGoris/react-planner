  ...
  _createClass(Vertex, null, [
    ...
    },
    {
      ...
      value: function beginDraggingVertex(state, layerID, vertexID, x, y) {
        var snapElements = _export.SnapSceneUtils.sceneSnapElements(
          state.scene,
          new _immutable.List(),
          state.snapMask
        )
    ...
    },
    {
      ...
      value: function updateDraggingVertex(state, x, y) {
        ...
        if (state.snapMask && !state.snapMask.isEmpty()) {
          snap = _export.SnapUtils.nearestSnap(
            snapElements,
            x,
            y,
            state.snapMask
      ...
      value: function endDraggingVertex(state, x, y) {
        ...
        if (lines) {
          state = lines.reduce(function (reducedState, lineID) {
            if (
              !reducedState.getIn(['scene', 'layers', layerID, 'lines', lineID])
            )
              return reducedState

            var v_id0 = reducedState.getIn([
              'scene',
              'layers',
              layerID,
              'lines',
            ...
            var oldVertex = reducedState.getIn([
              'scene',
              'layers',
              layerID,
              'vertices',
            ...
            var holes = reducedState
              ...
              .forEach(function (holeID) {
                ...
                var oldLineLength = _export.GeometryUtils.pointsDistance(
                  oldVertex.x,
                  oldVertex.y,
                  vertex.x,
                  vertex.y
                )
                var offset = _export.GeometryUtils.samePoints(
                  orderedVertices[1],
                  reducedState.getIn([
                    'scene',
                    'layers',
                  ...
                  ? 1 - hole.offset
                  : hole.offset
                var offsetPosition = _export.GeometryUtils.extendLine(
                  oldVertex.x,
                  oldVertex.y,
                  vertex.x,
                  vertex.y,
            ...
            var lineType = reducedState.getIn([
              'scene',
              'layers',
              layerID,
              'lines',
            ...
            var lineProps = reducedState.getIn([
              'scene',
              'layers',
              layerID,
              'lines',
            ...
            reducedState = _export2.Layer.removeZeroLengthLines(
              reducedState,
              layerID
            ).updatedState
            reducedState = _export2.Layer.mergeEqualsVertices(
              reducedState,
              layerID,
              vertexID
            ).updatedState
            reducedState = _export2.Line.remove(reducedState, layerID, lineID)
              .updatedState

            if (!_export.GeometryUtils.samePoints(oldVertex, vertex)) {
              var ret = _export2.Line.createAvoidingIntersections(
                reducedState,
                layerID,
                lineType,
                oldVertex.x,
              ...
              ret.lines.forEach(function (addedLine) {
                lineGroups.forEach(function (oldLineGroup) {
                  reducedState = _export2.Group.addElement(
                    reducedState,
                    oldLineGroup.id,
                    layerID,
                    'lines',
