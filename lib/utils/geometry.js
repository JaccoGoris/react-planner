...
function twoLineSegmentsIntersection(p1, p2, p3, p4) {
  ...
  if ((0, _math.fAbs)(denom) <= _constants.EPSILON) {
    if (
      (0, _math.fAbs)(numA) <= _constants.EPSILON &&
      (0, _math.fAbs)(numB) <= _constants.EPSILON
    ) {
  ...
  if (
    uA >= 0 - _constants.EPSILON &&
    uA <= 1 + _constants.EPSILON &&
    uB >= 0 - _constants.EPSILON &&
    uB <= 1 + _constants.EPSILON
  ) {
    var point = {
      x: x1 + uA * (x2 - x1),
      y: y1 + uA * (y2 - y1),
...
function samePoints(_ref, _ref2) {
  ...
  return (
    (0, _math.fAbs)(x1 - x2) <= _constants.EPSILON &&
    (0, _math.fAbs)(y1 - y2) <= _constants.EPSILON
  )
}
...
function extendLine(x1, y1, x2, y2, newDistance) {
  var precision =
    arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : 6
...
function verticesMidPoint(verticesArray) {
  var res = verticesArray.reduce(
    function (incr, vertex) {
